...
{#if isOpen}
    <div role="dialog" class="modal">
        <div class="contents">
            ...
            <div class="footer">
                <div class="actions">
                    <a class="button" href="/{$page.params.school}/cart" on:click={modals.close()}>Go to cart</a>
                    <button on:click={modals.close()}>Keep shopping</button>
                </div>
                ...
                <span class="cart">
                    {#if school}
                        ...
                        <a href="/{school}/cart" on:click={modals.close()}>{$cartCount} items</a> ${$cartSubtotal.toFixed(2)}                
                    {/if}
                </span>
            </div>
        </div>
