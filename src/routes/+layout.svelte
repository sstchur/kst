    ...
    onMount(() => {
        ...
        const observer = new IntersectionObserver((entries) => {
            for (const entry of entries) {
                ...
                kstText!.style.visibility = intersecting ? 'hidden' : 'visible';
            };
        }, { threshold: .25 });

        const logo = document.getElementById('kstLogo');
        observer.observe(logo!);
    });
</script>

<nav>    
    <span id="kstText">Kick Serve Tennis</span>
    <span>
        {#if school}
            <a href="/{school}/cart">Cart</a> <span>({$cartCount})</span>
        {/if}
...
<style>
    ...
    nav {
        ...
        background: #507599;
        padding: 1em;
        border-bottom: 2px solid #729349;
        color: #fff;
        font-size: 1.25em;
        position: fixed;
        width: 100%;
